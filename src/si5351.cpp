...
static void i2cWrite(uint8_t reg, uint8_t val) {   // write reg via i2c
  Wire.beginTransmission(I2C_VFO);
  Wire.write(reg);
  Wire.write(val);
  Wire.endTransmission();
}

static void i2c_write_blocking( uint8_t *vals, uint8_t vcnt) {
  Wire.beginTransmission(I2C_VFO);
